...
# === effort_trufor_wrapper auto-import ===
import ForensicHub.tasks.deepfake.wrapper.effort_trufor_wrapper  # noqa: F401
...
import ForensicHub.tasks.aigc.transforms
...
def main(args, model_args, train_dataset_args, test_dataset_args, transform_args, evaluator_args):
    global_rank = 0  # patched: default rank for non-distributed
    # init parameters for distributed training
    misc.init_distributed_mode(args)
    # --- patch: init a single-process dist group for evaluators that call dist.all_gather ---
    if (not args.distributed) and dist.is_available() and (not dist.is_initialized()):
        os.environ.setdefault("MASTER_ADDR", "127.0.0.1")
        # pick a port that is NOT busy (change this if you see EADDRINUSE)
        ...
        print("[patch] initialized torch.distributed process group (single-process, gloo)")
    # --- end patch ---
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.test_period == 0 or epoch + 1 == args.epochs:
            ...
            #              'epoch': epoch, }
            # Log metrics from ALL test datasets (not just the last one)
            log_stats = {**{f'train_{k}': v for k, v in train_stats.items()}, 'epoch': epoch}
            first_ds = True
            for ds_name, ds_metrics in values.items():
                prefix = "test" if first_ds else ds_name.lower()
                first_ds = False
                for k, v in ds_metrics.items():
                    log_stats[f"{prefix}_{k}"] = v
