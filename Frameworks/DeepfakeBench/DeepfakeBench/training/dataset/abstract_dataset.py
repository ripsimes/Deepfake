...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        # DiffusionForensics: flat JSON format [{path, label}]
        if dataset_name == 'DiffusionForensics':
            json_map = {
                'train': '/content/DiffusionForensics_Recons/train.json',
                'test':  '/content/DiffusionForensics_Recons/val.json',
            }
            with open(json_map[self.mode], 'r') as f:
                items = json.load(f)
            label_name_map = {0: 'DiffusionForensics_real', 1: 'DiffusionForensics_fake'}
            frame_path_list = [item['path'] for item in items]
            label_list      = [item['label'] for item in items]
            video_name_list = [f"diffusion_{i}" for i in range(len(items))]
